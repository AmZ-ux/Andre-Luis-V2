...
describe('DELETE /api/monthly-fees/:id — paid fee integrity', () => {
  it('paid fee without payment → DELETE returns 409, fee preserved', async () => {
    const db = getDb()
    const pid = uuid()
    db.prepare("INSERT INTO passengers (id, name, cpf, birth_date, transport_type, status, monthly_fee, due_day) VALUES (?, ?, ?, ?, 'university', 'active', 400, 5)")
      .run(pid, 'PaidNoPay', '111.111.111-11', '2000-01-01')
    const feeId = uuid()
    db.prepare("INSERT INTO monthly_fees (id, passenger_id, passenger_name, cpf, transport_type, month, year, amount, due_day, due_date, status) VALUES (?, ?, ?, ?, 'university', 9, 2026, 400, 5, '05/09/2026', 'paid')")
      .run(feeId, pid, 'PaidNoPay', '111.111.111-11')
    const res = await request(app)
      .delete(`/api/monthly-fees/${feeId}`)
      .set('Authorization', `Bearer ${token}`)
    expect(res.status).toBe(409)
    expect(res.body.error).toContain('pagas não podem ser excluídas')
    const fee = db.prepare('SELECT * FROM monthly_fees WHERE id = ?').get(feeId)
    expect(fee).toBeDefined()
  })

  it('paid fee with NORMAL payment → DELETE returns 409, fee + payment preserved', async () => {
    const db = getDb()
    const pid = uuid()
    db.prepare("INSERT INTO passengers (id, name, cpf, birth_date, transport_type, status, monthly_fee, due_day) VALUES (?, ?, ?, ?, 'university', 'active', 400, 5)")
      .run(pid, 'PaidWithPay', '222.222.222-22', '2000-01-01')
    const feeId = uuid()
    db.prepare("INSERT INTO monthly_fees (id, passenger_id, passenger_name, cpf, transport_type, month, year, amount, due_day, due_date, status) VALUES (?, ?, ?, ?, 'university', 9, 2026, 400, 5, '05/09/2026', 'paid')")
      .run(feeId, pid, 'PaidWithPay', '222.222.222-22')
    const payId = uuid()
    db.prepare("INSERT INTO payments (id, monthly_fee_id, amount, payment_date, payment_method, notes, late_fee, interest) VALUES (?, ?, 400, '05/09/2026', 'pix', '', 0, 0)")
      .run(payId, feeId)
    const res = await request(app)
      .delete(`/api/monthly-fees/${feeId}`)
      .set('Authorization', `Bearer ${token}`)
  ...
  it('paid fee with multiple payment records → DELETE returns 409, all preserved', async () => {
    const db = getDb()
    const pid = uuid()
    db.prepare("INSERT INTO passengers (id, name, cpf, birth_date, transport_type, status, monthly_fee, due_day) VALUES (?, ?, ?, ?, 'university', 'active', 400, 5)")
      .run(pid, 'PaidMultiPay', '333.333.333-33', '2000-01-01')
    const feeId = uuid()
    db.prepare("INSERT INTO monthly_fees (id, passenger_id, passenger_name, cpf, transport_type, month, year, amount, due_day, due_date, status) VALUES (?, ?, ?, ?, 'university', 9, 2026, 400, 5, '05/09/2026', 'paid')")
      .run(feeId, pid, 'PaidMultiPay', '333.333.333-33')
    const payId1 = uuid()
    db.prepare("INSERT INTO payments (id, monthly_fee_id, amount, payment_date, payment_method, notes, late_fee, interest, entry_type) VALUES (?, ?, 200, '05/09/2026', 'pix', 'parcial 1', 0, 0, 'normal')")
      .run(payId1, feeId)
    const payId2 = uuid()
    db.prepare("INSERT INTO payments (id, monthly_fee_id, amount, payment_date, payment_method, notes, late_fee, interest, entry_type) VALUES (?, ?, 200, '06/09/2026', 'pix', 'parcial 2', 0, 0, 'normal')")
      .run(payId2, feeId)
    const res = await request(app)
      .delete(`/api/monthly-fees/${feeId}`)
      .set('Authorization', `Bearer ${token}`)
  ...
  it('paid fee with pix_charge → DELETE returns 409, pix_charge preserved', async () => {
    const db = getDb()
    const pid = uuid()
    db.prepare("INSERT INTO passengers (id, name, cpf, birth_date, transport_type, status, monthly_fee, due_day) VALUES (?, ?, ?, ?, 'university', 'active', 400, 5)")
      .run(pid, 'PaidWithPix', '444.444.444-44', '2000-01-01')
    const feeId = uuid()
    db.prepare("INSERT INTO monthly_fees (id, passenger_id, passenger_name, cpf, transport_type, month, year, amount, due_day, due_date, status) VALUES (?, ?, ?, ?, 'university', 9, 2026, 400, 5, '05/09/2026', 'paid')")
      .run(feeId, pid, 'PaidWithPix', '444.444.444-44')
    const pixId = uuid()
    db.prepare("INSERT INTO pix_charges (id, payment_intent_id, monthly_fee_id, amount, status) VALUES (?, 'pi_test', ?, 400, 'approved')")
      .run(pixId, feeId)
    const res = await request(app)
      .delete(`/api/monthly-fees/${feeId}`)
      .set('Authorization', `Bearer ${token}`)
  ...
  it('DELETE paid fee does not modify any fields before 409', async () => {
    const db = getDb()
    const pid = uuid()
    db.prepare("INSERT INTO passengers (id, name, cpf, birth_date, transport_type, status, monthly_fee, due_day) VALUES (?, ?, ?, ?, 'university', 'active', 400, 5)")
      .run(pid, 'PaidNoMutate', '555.555.555-55', '2000-01-01')
    const feeId = uuid()
    db.prepare("INSERT INTO monthly_fees (id, passenger_id, passenger_name, cpf, transport_type, month, year, amount, due_day, due_date, status, notes) VALUES (?, ?, ?, ?, 'university', 9, 2026, 400, 5, '05/09/2026', 'paid', 'original notes')")
      .run(feeId, pid, 'PaidNoMutate', '555.555.555-55')
    const payId = uuid()
    db.prepare("INSERT INTO payments (id, monthly_fee_id, amount, payment_date, payment_method) VALUES (?, ?, 400, '05/09/2026', 'pix')")
      .run(payId, feeId)
    const pixId = uuid()
    db.prepare("INSERT INTO pix_charges (id, payment_intent_id, monthly_fee_id, amount, status) VALUES (?, 'pi_test', ?, 400, 'approved')")
      .run(pixId, feeId)
    ...
    expect(fee.amount).toBe(400)
    expect(fee.status).toBe('paid')
    expect(fee.notes).toBe('original notes')
    const payment = db.prepare('SELECT * FROM payments WHERE monthly_fee_id = ?').get(feeId)
    expect(payment).toBeDefined()
    const pix = db.prepare('SELECT * FROM pix_charges WHERE monthly_fee_id = ?').get(feeId)
    expect(pix).toBeDefined()
  ...
  it('pending fee DELETE behavior preserved (204, deletes fee + payments)', async () => {
    const db = getDb()
    const pid = uuid()
    db.prepare("INSERT INTO passengers (id, name, cpf, birth_date, transport_type, status, monthly_fee, due_day) VALUES (?, ?, ?, ?, 'university', 'active', 400, 5)")
      .run(pid, 'PendingDel', '666.666.666-66', '2000-01-01')
    const feeId = uuid()
    db.prepare("INSERT INTO monthly_fees (id, passenger_id, passenger_name, cpf, transport_type, month, year, amount, due_day, due_date, status) VALUES (?, ?, ?, ?, 'university', 9, 2026, 400, 5, '05/09/2026', 'pending')")
      .run(feeId, pid, 'PendingDel', '666.666.666-66')
    const payId = uuid()
    db.prepare("INSERT INTO payments (id, monthly_fee_id, amount, payment_date, payment_method) VALUES (?, ?, 400, '05/09/2026', 'pix')")
      .run(payId, feeId)
    const res = await request(app)
  ...
  it('overdue fee DELETE behavior preserved (204)', async () => {
    const db = getDb()
    const pid = uuid()
    db.prepare("INSERT INTO passengers (id, name, cpf, birth_date, transport_type, status, monthly_fee, due_day) VALUES (?, ?, ?, ?, 'university', 'active', 400, 5)")
      .run(pid, 'OverdueDel', '777.777.777-77', '2000-01-01')
    const feeId = uuid()
    db.prepare("INSERT INTO monthly_fees (id, passenger_id, passenger_name, cpf, transport_type, month, year, amount, due_day, due_date, status) VALUES (?, ?, ?, ?, 'university', 9, 2026, 400, 5, '05/09/2026', 'overdue')")
      .run(feeId, pid, 'OverdueDel', '777.777.777-77')
    const res = await request(app)
      .delete(`/api/monthly-fees/${feeId}`)
      .set('Authorization', `Bearer ${token}`)
    expect(res.status).toBe(204)
  ...
  it('cancelled fee DELETE behavior preserved (204)', async () => {
    const db = getDb()
    const pid = uuid()
    db.prepare("INSERT INTO passengers (id, name, cpf, birth_date, transport_type, status, monthly_fee, due_day) VALUES (?, ?, ?, ?, 'university', 'active', 400, 5)")
      .run(pid, 'CancelledDel', '888.888.888-88', '2000-01-01')
    const feeId = uuid()
    db.prepare("INSERT INTO monthly_fees (id, passenger_id, passenger_name, cpf, transport_type, month, year, amount, due_day, due_date, status) VALUES (?, ?, ?, ?, 'university', 9, 2026, 400, 5, '05/09/2026', 'cancelled')")
      .run(feeId, pid, 'CancelledDel', '888.888.888-88')
    const res = await request(app)
      .delete(`/api/monthly-fees/${feeId}`)
      .set('Authorization', `Bearer ${token}`)
    expect(res.status).toBe(204)
  ...
  it('exempt fee DELETE behavior preserved (204)', async () => {
    const db = getDb()
    const pid = uuid()
    db.prepare("INSERT INTO passengers (id, name, cpf, birth_date, transport_type, status, monthly_fee, due_day) VALUES (?, ?, ?, ?, 'university', 'active', 400, 5)")
      .run(pid, 'ExemptDel', '999.999.999-99', '2000-01-01')
    const feeId = uuid()
    db.prepare("INSERT INTO monthly_fees (id, passenger_id, passenger_name, cpf, transport_type, month, year, amount, due_day, due_date, status) VALUES (?, ?, ?, ?, 'university', 9, 2026, 400, 5, '05/09/2026', 'exempt')")
      .run(feeId, pid, 'ExemptDel', '999.999.999-99')
    const res = await request(app)
      .delete(`/api/monthly-fees/${feeId}`)
      .set('Authorization', `Bearer ${token}`)
    expect(res.status).toBe(204)
